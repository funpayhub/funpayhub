...
class RepositoriesManager:
    ...
    async def _load_repositories(self) -> None:
        logger.info(_('Loading plugins repositories from %s...'), str(self._repositories_path))
        ...
        for i in self._repositories_path.iterdir():
            if i.is_dir():
                logger.debug(_('%s is a directory. Skipping.'), str(i))
                continue
            if not i.suffix == '.json':
                logger.warning(_('%s is not a JSON file. Skipping.'), str(i))
                continue

            logger.info(_('Loading plugins repository from %s...'), str(i))
            try:
                repo = await FileRepositoryLoader(i).load()
                if repo.id != i.stem:
                    logger.warning(
                        _('Repository ID %s does not match repo file %s. Skipping.'),
                        repo.id,
                        i.name,
                    )
                    continue
                ...
                if isinstance(e, TranslatableException):
                    ...
                    logger.error(
                        _('An error occurred while loading plugin repository from %s.'),
                        str(i),
                        exc_info=True,
                    )
