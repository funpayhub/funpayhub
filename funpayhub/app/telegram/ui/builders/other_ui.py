...
class StartNotificationMenuBuilder(
    ...
    async def build(
        self, ctx: MenuContext, translater: Translater, hub: FunPayHub
    ) -> Menu:
        kb = KeyboardBuilder()
...
class MainMenuBuilder(MenuBuilder, menu_id=MenuIds.main_menu, context_type=MenuContext):
    async def build(self, ctx: MenuContext, translater: Translater, hub: FunPayHub):
        history = (
            ctx.callback_data.as_history() if ctx.callback_data is not None else []
        )
        ...
        kb.add_callback_button(
            ...
            callback_data=OpenMenu(
                menu_id=MenuIds.formatters_list, history=history
            ).pack(),
        )
        kb.add_callback_button(
            ...
            callback_data=OpenMenu(
                menu_id=MenuIds.goods_sources_list, history=history
            ).pack(),
        )
        kb.add_callback_button(
            ...
            callback_data=OpenMenu(
                menu_id=MenuIds.plugins_list, history=history
            ).pack(),
        )
...
class StateMenuBuilder(
    MenuBuilder, menu_id=MenuIds.state_menu, context_type=StateUIContext
):
    async def build(self, ctx: StateUIContext, translater: Translater) -> Menu:
        ...
        kb.add_callback_button(
            ...
            callback_data=ClearState(
                ...
                history=(
                    ctx.callback_data.history if ctx.callback_data is not None else []
                ),
            ).pack(),
        )
...
class RequestsMenuBuilder(
    MenuBuilder, menu_id="fph:request_menu", context_type=MenuContext
):
...
class NewReviewNotificationMenuBuilder(
    ...
    async def build(
        self, ctx: NewReviewNotificationMenuContext, translater: Translater
    ) -> Menu:
