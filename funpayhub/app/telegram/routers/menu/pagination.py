...
async def change_page(query: CallbackQuery, callback_data: cbs.ChangePageTo, tg: Telegram) -> None:
    ...
    await tg.execute_previous_callback(old.pack(), query)
...
async def activate_manual_page_changing_state(
    ...
    await state.set_data({'data': data_obj})
    await query.answer()


@router.message(StateFilter(ChangingMenuPage.identifier))
@router.message(StateFilter(ChangingViewPage.identifier))
async def change_page_from_message(message: Message, state: FSMContext, tg: Telegram) -> None:
    ...
    data: ChangingViewPage | ChangingMenuPage = (await state.get_data())['data']
    ...
    old.data['view_page' if isinstance(data, ChangingViewPage) else 'menu_page'] = new_page_index
    old.history = data.callback_data.history[:-1]
    await tg.execute_previous_callback(callback_data=old.pack(), query=data.callback_query_obj)
