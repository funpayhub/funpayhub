...
from funpayhub.lib.base_app.telegram.app.properties.ui import (
    NodeMenuContext as NodeMenuCtx,
)
...
class AddAutoDeliveryRuleMenuBuilder(
    ...
    async def build(self, ctx: MenuContext, translater: Tr, hub: FPH) -> Menu:
        ...
        if hub.funpay.authenticated:
            ...
            for offer in chain(
                k for i in profile.offers.values() for j in i.values() for k in j
            ):
...
class AutoDeliveryGoodsSourcesListMenuBuilder(
    ...
    async def build(
        self, ctx: NodeMenuCtx, goods_manager: GoodsManager, translater: Tr
    ) -> Menu:
        kb = KeyboardBuilder()
...
class NewSaleNotificationMenuBuilder(
    MenuBuilder, menu_id=MenuIds.new_sale_notification, context_type=NewSaleMenuContext
):
    async def build(self, ctx: NewSaleMenuContext, translater: Tr) -> Menu:
        ...
        if delivered_goods := ctx.new_sale_event.data.get("delivered_goods"):
            menu.footer_text = (
                f"<i>📦 Успешно доставлено {len(delivered_goods)} товаров.</i>"
            )
        ...
        menu.header_keyboard.add_callback_button(
            ...
            callback_data=SendMessage(
                to=ctx.new_sale_event.message.chat_id, name=order.counterparty.username
            ).pack_compact(),
        )
...
class ReplaceSourcesListButtonModification(
    ...
    async def filter(self, ctx: NodeMenuCtx, menu: Menu, properties: FPHProps) -> bool:
        return (
            ...
            and ctx.entry_path[0] == properties.auto_delivery.id
        )
...
class AddRemoveButtonToAutoDeliveryModification(
    ...
    async def modify(self, ctx: NodeMenuCtx, menu: Menu, translater: Tr) -> Menu:
        ...
        return await self._modify(
            ctx, menu, translater, delete_callback=delete_callback
        )
